...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    // AP+STA mode needs a default netif for each Wi-Fi interface.  The STA
    // netif owns the DHCP client and emits IP_EVENT_STA_GOT_IP.
...
bool WifiConfigurationAp::ConnectToWifi(const std::string &ssid, const std::string &password)
{
    ...
    // Wait until the station has received an IP address. Association alone is
    // not sufficient to prove that the configured network is usable.
