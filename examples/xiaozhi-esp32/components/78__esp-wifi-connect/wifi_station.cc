...
constexpr int64_t WIFI_SCAN_RETRY_INTERVAL_US = 10 * 1000 * 1000;
...
void WifiStation::HandleScanResult() {
    ...
    if (connect_queue_.empty()) {
        ...
        esp_timer_start_once(timer_handle_, WIFI_SCAN_RETRY_INTERVAL_US);
        return;
    }
...
void WifiStation::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    if (event_id == WIFI_EVENT_STA_START) {
        ...
        esp_timer_start_once(this_->timer_handle_, WIFI_SCAN_RETRY_INTERVAL_US);
