...
esp_err_t vocat_base_control_adjust_angle(int delta_angle)
{
    if (delta_angle < -180 || delta_angle > 180) {
        ESP_LOGE(TAG, "Relative angle out of range: %d", delta_angle);
        return ESP_ERR_INVALID_ARG;
    }

    ESP_LOGI(TAG, "Adjust echo base angle: %+d", delta_angle);
    return vocat_base_control_send_command_frame(
        VOCAT_BASE_CMD_SET_RELATIVE_ANGLE, (uint16_t)(int16_t)delta_angle);
}
